...
Script.wonders = (function(){
	...
	instance.wonders["dyson"] = {cost:{"energy":100000, "plasma":10000}, available:false, done:false, mk:unlockDysonResearch, unlocks:[], consequences:function(){Script.science.techs["unlockDyson"].available = true;}, skip:function(){return Game.tech.isPurchased("unlockDyson");}};
	...
	instance.update = function(self)
	{
		...
		for (id in self.wonders)
		{
			...
			if (wonder.available && !wonder.done)
			{
				...
				{
					var mult = 1;
					var amount = wonder.cost[key] / (10 * (1 + Math.log10(wonder.cost[key]) * Math.log10(wonder.cost[key]) * 4));
					...
					if (production * 240 < (cost - getResource(key))) {rush = false;}
					if (cost > getStorage(key)) {rush = false;}
				}
				if (rush)
				{
