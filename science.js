...
Script.science = (function(){
	...
	instance.techs["unlockBasicEnergy"] = {available:true, done:false, unlocks:["unlockSolar", "unlockMachines", "upgradeEngineTech"], consequences:function(){Script.data.unlockResource(Script.data, "charcoal"); Script.energy.data["engine"].unlocked = true;}};
	...
	instance.techs["unlockSolar"] = {available:false, done:false, unlocks:["upgradeSolarTech"], consequences:function(){Script.energy.data["solar"].unlocked = true;}};
	...
	instance.update = function(self)
	{
		...
		for (id = 0; id < Script.labTier; id++)
		{
			...
			for (key in building.cost) {Script.cost.addCost(Script.cost, key, addition * Math.sqrt(Math.sqrt(result.cost[key])));}
		}
